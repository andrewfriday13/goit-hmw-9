...
function submitForm(evt) {
    ...
    for(let i = 0; i < amount.value; i += 1){
        ...
        createPromise(index, firstStep).then(({ position , delay: delay1  })=>{
            (0, _notiflixDefault.default).Notify.success(`✅ Fulfilled promise ${position} in ${delay1}ms`);
        }).catch(({ position , delay: delay2  })=>{
            (0, _notiflixDefault.default).Notify.failure(`❌ Rejected promise ${position} in ${delay2}ms`);
        });
    }
    evt.target.reset();
}
